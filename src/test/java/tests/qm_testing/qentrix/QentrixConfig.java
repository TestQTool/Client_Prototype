package tests.qm_testing.qentrix;
...
public final class QentrixConfig {
    ...
    static {
        try (InputStream input = QentrixConfig.class.getClassLoader().getResourceAsStream("qm_testing/qentrix/config.properties")) {
            if (input != null) {
                PROPERTIES.load(input);
            }
        } catch (IOException e) {
            throw new IllegalStateException("Unable to load qm_testing/qentrix/config.properties", e);
        }
    }
