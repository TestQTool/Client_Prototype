...
public final class QentrixConfig {
    ...
    static {
        try (InputStream input = QentrixConfig.class.getClassLoader().getResourceAsStream("super_admin_organization/orangehrm/config.properties")) {
            if (input != null) {
                PROPERTIES.load(input);
            }
        } catch (IOException e) {
            throw new IllegalStateException("Unable to load super_admin_organization/orangehrm/config.properties", e);
        }
    }
